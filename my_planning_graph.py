...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return not(set(node_a1.action.effect_add).isdisjoint(node_a2.action.effect_rem) and set(node_a2.action.effect_add).isdisjoint(node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return not(set(node_a1.action.effect_add).isdisjoint(node_a2.action.precond_neg) and set(node_a1.action.effect_rem).isdisjoint(node_a2.action.precond_pos) and set(node_a2.action.effect_add).isdisjoint(node_a1.action.precond_neg) and set(node_a2.action.effect_rem).isdisjoint(node_a1.action.precond_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any((a1P.is_mutex(a2P) for a1P in node_a1.parents for a2P in node_a2.parents))
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all((s1P.is_mutex(s2P) for s1P in node_s1.parents for s2P in node_s2.parents))
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            level_sum += next(level for level, sNodes in enumerate(self.s_levels) if PgNode_s(goal, True) in sNodes)
